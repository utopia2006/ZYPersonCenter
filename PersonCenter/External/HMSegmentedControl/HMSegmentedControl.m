...
- (id)initWithSectionTitles:(NSArray *)sectiontitles sectionBadges:(NSMutableArray *)sectionBadges {
    self = [self initWithSectionTitles:sectiontitles];
    
    if (self) {
        self.sectionBadges = sectionBadges;
        self.badgeImage = [UIImage imageNamed:@"tab_red.png"];
    }
...
- (void)setBadge:(BOOL)badge segmentIndex:(NSUInteger)index {
    if ([self.sectionBadges count] == 0) {
        return;
    }
    
    [self.sectionBadges replaceObjectAtIndex:index withObject:[NSNumber numberWithBool:badge]];
    [self setNeedsDisplay];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        ...
        // 显示红色标记
        [self.sectionBadges enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = 0;
            CGFloat stringHeight = 0;
            CGSize size = [self measureTitleAtIndex:idx];
            stringWidth = size.width;
            ...
            CGFloat badgeWidth = 9.0f;
            CGFloat badgeHeight = 9.0f;
            
            CGFloat y = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - stringHeight / 2 );
            CGRect rect;
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                rect = CGRectMake((self.segmentWidth * idx) + (self.segmentWidth - stringWidth) / 2 + stringWidth - badgeWidth/2, y, badgeWidth, badgeHeight);
            } else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                CGFloat xOffset = 0;
                NSInteger i = 0;
                ...
                CGFloat widthForIndex = [[self.segmentWidthsArray objectAtIndex:idx] floatValue];
                rect = CGRectMake(xOffset + widthForIndex - badgeWidth/2, y, badgeWidth, badgeHeight);
            }
            
            // Fix rect position/size to avoid blurry labels
            rect = CGRectMake(ceilf(rect.origin.x), ceilf(rect.origin.y), ceilf(rect.size.width), ceilf(rect.size.height));
            
            if ([(NSNumber *)obj boolValue] && self.badgeImage) {
                CALayer *badgeLayer = [CALayer layer];
                badgeLayer.frame = rect;
                badgeLayer.contents = (id)self.badgeImage.CGImage;
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if ([self.sectionBadges count]) {
        [self.sectionBadges replaceObjectAtIndex:index withObject:[NSNumber numberWithBool:NO]];
    }
    [self setNeedsDisplay];
