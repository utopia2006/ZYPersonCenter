...
static char Refresh_Key, ScrollView_Key, Block_Key, MarginTop_Key, Animation_Key, RefreshStatus_Key,scrollerOriginY_Key,refreshStartY_Key,timer_Key;

@implementation UIView (XDWRefresh)

- (void)setTimers:(NSTimer *)timers{
        objc_setAssociatedObject(self, &timer_Key, timers, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSTimer *)timers{
...
/**freshView**/
- (void)setRefreshView:(XDRefreshView *)refreshView {
    objc_setAssociatedObject(self, &Refresh_Key, refreshView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (XDRefreshView *)refreshView {
    return objc_getAssociatedObject(self, &Refresh_Key);
}

/**SETTER scrollerOriginY**/
- (void)setScrollerOriginY:(CGFloat)scrollerOriginY{
    objc_setAssociatedObject(self, &scrollerOriginY_Key, @(scrollerOriginY), OBJC_ASSOCIATION_ASSIGN);
}
-(CGFloat)scrollerOriginY{
    return [objc_getAssociatedObject(self, &scrollerOriginY_Key) floatValue];
}
/**SETTER scrollerOriginY**/
- (void)setRefreshStartY:(CGFloat)refreshStartY{
//    objc_setAssociatedObject(self, &refreshStartY_Key, @(refreshStartY), OBJC_ASSOCIATION_ASSIGN);
        objc_setAssociatedObject(self, &refreshStartY_Key, @(refreshStartY), OBJC_ASSOCIATION_RETAIN);
}
- (CGFloat)refreshStartY{
    return [objc_getAssociatedObject(self, &refreshStartY_Key) floatValue];
}
...
- (void)setMarginTop:(CGFloat)marginTop {
    objc_setAssociatedObject(self, &MarginTop_Key, @(marginTop), OBJC_ASSOCIATION_RETAIN);
//     objc_setAssociatedObject(self, &MarginTop_Key, @(marginTop), OBJC_ASSOCIATION_ASSIGN);
}
- (CGFloat)marginTop {
...
- (void)setRefreshStatus:(StatusOfRefresh)refreshStatus {
    objc_setAssociatedObject(self, &RefreshStatus_Key, [NSNumber numberWithInteger:refreshStatus], OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)XD_refreshWithObject:(UIScrollView *)scrollView atPoint:(CGPoint)position downRefresh:(void (^)(void))block {
    ...
    if (!self.refreshView) {
        ...
        self.refreshView = [[XDRefreshView alloc]initWithFrame:positionFrame];
    }
...
- (void)defaultHandleWithOffSet:(CGFloat)offsetY change:(NSDictionary<NSKeyValueChangeKey,id> *)change {
    ...
    self.refreshStartY<0?self.refreshStartY=0:self.refreshStartY;
    /**刷新动作区间**/
    if (defaultoffsetY > (self.threshold+self.scrollerOriginY-self.refreshStartY) && defaultoffsetY < self.scrollerOriginY-self.refreshStartY) {
        [self.refreshView setContentOffset:CGPointMake(0, defaultoffsetY-self.scrollerOriginY+self.refreshStartY)];
    ...
    /**(@"刷新临界点，把刷新icon置为最大区间")**/
    if (defaultoffsetY <= (self.threshold+self.scrollerOriginY-self.refreshStartY) && self.refreshView.contentOffset.y != (self.threshold+self.scrollerOriginY-self.refreshStartY)) {
    ...
    if (defaultoffsetY <= (self.threshold+self.scrollerOriginY-self.refreshStartY) && self.refreshView.contentOffset.y == self.threshold) {
    ...
    if (defaultoffsetY >= self.scrollerOriginY-self.refreshStartY && self.refreshView.contentOffset.y != 0) {
...
- (void)endRefresh {
    ...
    if (self.extenScrollView.isDragging) {
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 10) {
          self.timers = [NSTimer scheduledTimerWithTimeInterval:0.2 repeats:YES block:^(NSTimer * _Nonnull timer) {
                [self endRefresh];
                [timer invalidate];
            }];
         ...
         self.timers = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self selector:@selector(timerCall:) userInfo:nil repeats:YES];
        }
...
- (void)XD_freeReFresh {
    NSLog(@"%p",self);
    //移除通知不应该在类别中，应该放到类别的这个view中去移除，不然会闪退
//    [[NSNotificationCenter defaultCenter] removeObserver:self.extenScrollView];
    self.refreshBlock = nil;
    self.animation = nil;
    self.refreshView.refreshIcon = nil;
    self.refreshView = nil;
    self.extenScrollView = nil;
    [self.refreshView endRefresh];

    [self.timers invalidate];
    self.timers = nil;
    [self.refreshView.layer removeAllAnimations];
    [self.refreshView.refreshIcon.layer removeAllAnimations];
}
@end
...
- (void)creatMainUI {
    ...
    _refreshIcon.image = [UIImage imageNamed:@"refresh"];
